...
Generated: `2026-09-17T21:00:01+00:00`
...
| Sources | 862 |
| Source documents with current assessments | 39 |
| Source documents assessed complete | 38 |
| Source documents with object-level extraction | 29 |
...
1. ACCESS-004 / SCHOL-005: finish the hash-bound document ledger by classifying the eleven remaining HTML-only captures as landing pages, item pages or substantive documents without inferring completeness from MIME type. All 39 PDF-linked sources now have rendered-page, hash-bound assessments.
...
- [ ] **ACCESS-004 — Create a document-completeness and transformation ledger** · In progress · Engineering
  ...
  - Evidence/status: Migration 015 and `ibi ingest-document-assessment` add an immutable, supersedable per-holding ledger that keeps document form and extent, inspection method, text transformation and object-level extraction separate. Every assessment binds the retained document, review evidence and any OCR/rich-text or extraction artifact by SHA-256. All 39 PDF-linked sources now have 40 holding assessments: 38 sources have a complete document and Naveh-Shaked 1993 remains correctly limited to front matter. Ford 2023 retains both its one-page summary as an excerpt and its separately archived complete article. The final PDF tranche adds the complete Waller chapter, museum and auction catalogues, Moriggi 2024, Brand's working list, Levene-Bhayro 2006, Kedar's dissertation and the full Schwab journal volume. Twenty-nine sources have hash-bound object-level extraction, twenty-two complete and seven partial. No scan is mislabeled OCR or corrected rich text, and Gordon's inspected-but-unarchived AASOR article is not counted as a holding. Only the eleven HTML-only captures remain explicit and unassessed.
