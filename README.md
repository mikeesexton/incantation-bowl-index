...
Saturation is bounded by the twelve source classes that were searched. Waller's 115-title list of JBA bowl publications (1853–2024) is used as a bibliography audit: every title now has a source record, so the project knows what it still needs to inspect. Bowl-level work is measured separately. The database currently resolves **26 publication editions covering 879 candidate records**, and its complete inspected corpus editions and catalogues supply hundreds of page-located source appearances. See the [control-list measurement](data/reports/waller_control_list_2026-09-05.md), the [current mission plan](docs/dataset_maturity_roadmap.md), and the [scoping review alignment](data/reports/scoping_review_alignment_2026-09-05.md).

## Identity and enrichment status

The current corpus contains **1,960 records representing 1,692 working identity hypotheses**, with 2,144 appearances from 862 sources. These are working research classifications, not an overall accuracy estimate. Complete inspected editions and catalogues now include both *Aramaic Bowl Spells* volumes, Montgomery, Moriggi's Syriac corpus, Ford–Morgenstern's Hilprecht catalogue, Levene's curse-text volume, the modern Berlin catalogue, Naveh–Shaked 1985, Pognon 1898–1899, Ellis's 1853 section, and Stübe 1895. Stübe adds nineteen historical Berlin appearances: seventeen attach directly to modern catalogue records by exact VA number, while two unnamed Pahlavi bowls remain provisional rather than being guessed onto later objects.

The public reading-room count is deliberately narrower than edition coverage. It currently exposes **35 scan-checked public-domain translations and 9 project-authored summaries**. Another 154 text records retain their citation and locator but withhold protected scholarly wording. Thus “44 readable text records” does not mean that only 44 bowls are known from editions: 195 objects have a stored text record, 49 have a translation record, and many more have edition-level references without a locally publishable transcription or translation. Three earlier own-work approvals no longer match the current content fingerprint and therefore fail closed instead of silently publishing changed text.
