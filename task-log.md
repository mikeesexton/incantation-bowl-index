...
**Corpus:** unchanged — state digest `591faac7157c`
**Tests:** Mac mini and independent local-Restic restore: 312 Bowl Index Python tests passed; IvritElite Node tests passed; SQLite integrity/quick/foreign-key checks passed; archive verification valid; restored corpus state matched; both repository checkouts clean
...
- Configured the 2 TB `IBI Backup` drive as encrypted APFS, generated a separate local-Restic password retained in the Mac mini login Keychain, and created snapshot `6b5ec6e564e4cf874ce38c46a464ee52fb8fd1f0e3d26558a455dafba4c2e0e1`. A full data-pack check completed without error. Added reviewed local backup and restore helpers plus an explicit cache/worktree exclude list; no private path is added to Git.
- Independently restored 1.746 GiB / 1,664 files and directories into a new temporary directory. The restored database SHA-256 was `9695ff17ba3439830c76590bb18171c5830a0c6f637cf3fe34e030d8eaafadb2`; SQLite integrity and quick checks returned `ok`, foreign-key violations were zero, every archive capture validated, corpus state matched, and both project test suites passed. Preserved the private receipt and removed only the reviewed temporary restore.
- Migration and the local recovery gate are complete, but OPS-001 and OPS-002 remain unaccepted. Tailscale is connected on the mini but the laptop was last observed offline; the separate standard account, B2 repository and restore, Healthchecks and schedules are not configured. All seventeen legacy SQLite snapshots remain retained.
